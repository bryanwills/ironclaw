...
- The widget identifier validator (`is_safe_widget_id` in `crates/product/ironclaw_gateway/src/layout.rs`, applied to discovery, serving, and `manifest.id`) and the `manifest.id == directory_name` check protect against widget-to-widget confusion, not against malicious-but-well-formed widgets.
