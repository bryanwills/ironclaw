...
- `crates/product/ironclaw_gateway/static/js/core/onboarding.js`: `handleOnboardingState(...)` as the canonical client entrypoint (the old monolithic `app.js` has been split into per-concern modules under `static/js/`; `APP_JS` in `crates/product/ironclaw_gateway/src/assets.rs` concatenates them at compile time)
