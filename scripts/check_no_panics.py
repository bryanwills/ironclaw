...
def is_test_only_path(path: str) -> bool:
    """Return True for Rust files that are test-only by repository convention.

    Files under ``src/**/tests/*.rs`` and module files named ``tests.rs`` are
    Rust test sub-modules, typically included behind ``#[cfg(test)]`` and never
    compiled in production. Top-level ``tests/*.rs`` integration test files are
    already outside ``src/`` / ``crates/`` and therefore never checked.
    """
    parsed = pathlib.PurePosixPath(path)
    return "tests" in parsed.parts or parsed.name == "tests.rs"
...
class CheckNoPanicsTests(unittest.TestCase):
    ...
    def test_test_only_path_detection(self) -> None:
        ...
        self.assertTrue(is_test_only_path("crates/foo/src/feature/tests.rs"))
