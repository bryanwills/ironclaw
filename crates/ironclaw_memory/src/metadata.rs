...
impl DocumentMetadata {
    /// Strict parse for the untrusted boundary (tool input): a present-but-
    /// malformed metadata blob is rejected rather than silently coerced to
    /// defaults (which would drop the caller's schema/context).
    pub fn try_from_value(value: &serde_json::Value) -> Result<Self, serde_json::Error> {
        serde_json::from_value(value.clone())
    }

    /// Resilient parse for persisted reads. A corrupt stored/inherited `.config`
    /// blob is logged and falls back to defaults so a single bad document cannot
    /// take down reads or listing — the underlying bytes are never deleted, only
    /// the parse is skipped. Untrusted input must use [`Self::try_from_value`].
