...
async fn trace_credits_reports_enrolled_for_caller_with_enabled_policy() {
    ...
    // The trace scope is the tenant-scoped key (tenant_id + user id), matching
    // the facade read path in `RebornServicesApi::trace_credits`. Write an
    // enabled standing policy for that exact key and assert the route reflects
    // enrollment for that caller only.
    let user_id = unique_trace_credits_user();
    let scope_key = trace_scope_key(TENANT, user_id.as_str());
    let _cleanup = TraceScopeCleanup(scope_key.clone());
    let policy = StandingTraceContributionPolicy {
        enabled: true,
        ..StandingTraceContributionPolicy::default()
    };
    write_trace_policy_for_scope(Some(scope_key.as_str()), &policy).expect("write trace policy");
