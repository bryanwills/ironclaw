//! Composition shims for the extracted Slack host crate.

use std::sync::Arc;

use ironclaw_product_workflow::ProductActorUserResolver;
use ironclaw_reborn_http_kit::PublicRouteMount;
use ironclaw_reborn_slack_host::{
    SlackHostBetaBuildError, SlackHostBetaConfig, SlackHostBetaMounts, SlackHostRuntimeHandles,
};

use crate::RebornRuntime;
use crate::factory::LocalDevRootFilesystem;

pub fn slack_host_runtime_handles_from_runtime(
    runtime: &RebornRuntime,
) -> Result<SlackHostRuntimeHandles<LocalDevRootFilesystem>, SlackHostBetaBuildError> {
    ...
    Ok(SlackHostRuntimeHandles::new(
        Arc::clone(&local_runtime.host_state_filesystem),
        host_runtime_http_egress,
        runtime.webui_thread_service(),
        runtime.webui_turn_coordinator(),
        runtime.webui_approval_interaction_service(),
        runtime.webui_auth_interaction_service(),
        runtime.auth_challenge_provider(),
    ))
}
...
pub fn build_slack_events_route_mount(
    ...
    let handles = slack_host_runtime_handles_from_runtime(runtime)?;
    ironclaw_reborn_slack_host::build_slack_events_route_mount_with_handles(&handles, config)
}
...
pub fn build_slack_host_beta_mounts(
    ...
    let handles = slack_host_runtime_handles_from_runtime(runtime)?;
    ironclaw_reborn_slack_host::build_slack_host_beta_mounts_with_handles(&handles, config)
}
...
pub fn build_slack_events_route_mount_with_actor_user_resolver(
    ...
    let handles = slack_host_runtime_handles_from_runtime(runtime)?;
    ironclaw_reborn_slack_host::build_slack_events_route_mount_with_actor_user_resolver_with_handles(
        &handles,
        config,
        actor_user_resolver,
    )
...
mod tests {
    ...
    use ironclaw_product_workflow::{
        OutboundDeliveryTargetProvider, ProductActorUserResolutionRequest, ProductWorkflowError,
        RebornChannelConnectStrategy, RebornOutboundDeliveryTargetId,
    ...
    use ironclaw_reborn_slack_host::{
        FilesystemSlackHostState, InMemorySlackChannelRouteStore,
        InMemorySlackPersonalDmTargetStore, SLACK_OUTBOUND_TARGET_LIST_PAGE_SIZE,
        SLACK_SIGNATURE_HEADER, SLACK_TIMESTAMP_HEADER, SlackChannelRoute, SlackChannelRouteError,
        SlackChannelRouteKey, SlackChannelRouteListPage, SlackChannelRouteStore,
        SlackConfiguredChannelRoute, SlackHostBetaActorUserResolver, SlackHostBetaChannelRoute,
        SlackHostBetaConfigInput, SlackHostBetaOutboundTargetProvider,
        ...
        SlackTeamId, SlackUserId, WEBUI_V2_CHANNELS_SLACK_ALLOWED_PATH,
        WEBUI_V2_CHANNELS_SLACK_ROUTES_PATH, WEBUI_V2_EXTENSION_PAIRING_REDEEM_PATH,
    ...
    impl SlackChannelRouteStore for NonAdvancingCursorRouteStore {
        ...
        async fn replace_managed_routes(
            ...
            _installation_id: &AdapterInstallationId,
            _team_id: &str,
            _assignments: Vec<ironclaw_reborn_slack_host::SlackChannelRouteAssignment>,
    ...
    fn personal_dm_target_provisioner_for_test(
        ...
        let handles = slack_host_runtime_handles_from_runtime(runtime).expect("Slack handles");
    ...
    impl SlackPersonalDmTargetStore for FailingSlackPersonalDmTargetStore {
        async fn load_personal_dm_target(
            &self,
            _key: &ironclaw_reborn_slack_host::SlackPersonalDmTargetKey,
        ) -> Result<
            Option<ironclaw_reborn_slack_host::SlackPersonalDmTarget>,
        ...
        async fn upsert_personal_dm_target(
            &self,
            target: ironclaw_reborn_slack_host::SlackPersonalDmTarget,
        ) -> Result<ironclaw_reborn_slack_host::SlackPersonalDmTarget, SlackPersonalDmTargetError>
        {
            Ok(target)
        }
    }
