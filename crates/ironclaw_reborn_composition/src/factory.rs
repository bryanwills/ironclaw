    ...
    F: RootFilesystem + 'static,
{
    ...
    // The production trust-invalidation bus is consumed exclusively by
    // `ActiveExtensionPublisher`; `HostRuntimeServices` has no equivalent
    // builder method. Grant revocations from trust-policy changes flow
    // through the publisher rather than the services layer.
