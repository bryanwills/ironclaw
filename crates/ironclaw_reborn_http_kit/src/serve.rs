...
//! Reborn-side home for that work: it exposes [`compose_webui_v2_app`],
//! the fully-composed axum [`Router`] (auth + rate limit + CORS + body
...
//!
//! `ironclaw_reborn_http_kit` sits in the Reborn product/API
...
/// caller — so the same `compose_webui_v2_app` works for the CLI binary
/// and for any future ingress fronting the same routes.
///
...
/// methods only, except for the host-identity / mount fields the
/// product-aware composition root (`ironclaw_reborn_composition::webui`)
/// reads when lowering product services into mounts. This routes every
/// host through `new` / `parse_allowed_origins` / `with_*`, which
/// fail-closed on invalid input (empty token, malformed origin, bad
/// CSP). The fail-closed defaults — empty allow-origin list, locked-down
/// CSP, 14 MiB outer body cap — apply unless an explicit builder
...
pub struct WebuiServeConfig {
    ...
    pub public_mounts: Vec<PublicRouteMount>,
    ...
    pub protected_mounts: Vec<ProtectedRouteMount>,
    ...
    pub google_oauth: Option<GoogleOAuthRouteConfig>,
}
...
pub struct ProtectedRouteMount {
    pub router: Router,
    pub descriptors: Vec<IngressRouteDescriptor>,
    /// Operator-gated mounts are composed only when the authenticator
    /// opts into operator WebUI config routes
    /// ([`WebuiAuthenticator::mounts_operator_webui_config_routes`]);
    /// when mounted, their descriptors also join the operator descriptor
    /// set used for per-request `operator_webui_config` authorization.
    operator_gated: bool,
}
...
impl ProtectedRouteMount {
    ...
    /// A protected mount that mutates operator-wide host configuration
    /// (provider catalogs, secrets, channel ownership). Dropped entirely
    /// — router and descriptors — unless the authenticator mounts
    /// operator WebUI config routes, and authorized per-request from the
    /// matched token's capabilities like the native v2 operator routes.
...
///
/// This function is product-agnostic: every non-v2 route family
/// (product auth, Slack, OpenAI-compat, SSO login) reaches it as a
/// [`ProtectedRouteMount`] / [`PublicRouteMount`] on
/// [`WebuiServeConfig`]. Product-aware composition (converting
/// `RebornWebuiBundle::product_auth` into mounts) lives in
/// `webui_v2_app` (in `ironclaw_reborn_composition`).
pub fn compose_webui_v2_app(
    ...
    // Operator-gated mounts are dropped entirely — router AND
    // descriptors — when the authenticator does not mount operator
    // WebUI config routes, mirroring the native v2 operator routes.
