...
fn reborn_cli_binary_crate_stays_separate_from_v1_root() {
    ...
    let root = workspace_root();
    let manifest_path = root.join("crates/agent/ironclaw_reborn_cli/Cargo.toml");
    assert!(
    ...
    let command_module_paths = [
        ...
        "crates/agent/ironclaw_reborn_cli/src/commands/mod.rs",
        "crates/agent/ironclaw_reborn_cli/src/commands/completion.rs",
        "crates/agent/ironclaw_reborn_cli/src/commands/doctor.rs",
        "crates/agent/ironclaw_reborn_cli/src/commands/run.rs",
        "crates/agent/ironclaw_reborn_cli/src/context.rs",
    ];
    ...
    let agent_contract = std::fs::read_to_string(root.join("crates/agent/ironclaw_reborn_cli/AGENTS.md"))
...
fn reborn_host_runtime_services_do_not_expose_lower_substrate_handles() {
    let root = workspace_root();
    let lib = std::fs::read_to_string(root.join("crates/runtime/ironclaw_host_runtime/src/lib.rs"))
        .expect("host runtime lib.rs must be readable");
    let services =
        std::fs::read_to_string(root.join("crates/runtime/ironclaw_host_runtime/src/services.rs"))
            .expect("host runtime services.rs must be readable");
    let scripts = std::fs::read_to_string(root.join("crates/runtime/ironclaw_scripts/src/lib.rs"))
        .expect("script runtime lib.rs must be readable");
    let scripts_manifest = std::fs::read_to_string(root.join("crates/runtime/ironclaw_scripts/Cargo.toml"))
        .expect("script runtime Cargo.toml must be readable");
    let mcp = std::fs::read_to_string(root.join("crates/runtime/ironclaw_mcp/src/lib.rs"))
        .expect("MCP runtime lib.rs must be readable");
    let mcp_manifest = std::fs::read_to_string(root.join("crates/runtime/ironclaw_mcp/Cargo.toml"))
...
fn reborn_turns_public_surface_keeps_runner_api_explicit() {
    let root = workspace_root();
    let lib = std::fs::read_to_string(root.join("crates/agent/ironclaw_turns/src/lib.rs"))
...
fn reborn_loop_support_llm_wiring_stays_out_of_root_src() {
    ...
    assert!(
        ...
        "Reborn loop LLM wiring must live under crates/agent/ironclaw_reborn, not root src/lib.rs"
    );
    assert!(
    ...
    let reborn_gateway = root.join("crates/agent/ironclaw_reborn/src/model_gateway.rs");
    assert!(
    ...
    assert!(
        ...
        "Reborn LLM gateway wiring should expose LlmProviderModelGateway from crates/agent/ironclaw_reborn"
    );

    let reborn_manifest = std::fs::read_to_string(root.join("crates/agent/ironclaw_reborn/Cargo.toml"))
...
fn reborn_turns_public_surface_uses_turn_ids_not_runtime_or_process_ids() {
    let root = workspace_root();
    let turns_src = root.join("crates/agent/ironclaw_turns/src");
...
fn wasm_product_adapter_crate_has_local_guardrails() {
    let guardrails = workspace_root().join("crates/product/ironclaw_wasm_product_adapters/CLAUDE.md");
    assert!(
        guardrails.exists(),
...
fn wasm_product_adapter_wit_preserves_product_adapter_trust_boundary() {
    let wit = std::fs::read_to_string(
        workspace_root().join("crates/product/ironclaw_wasm_product_adapters/wit/product_adapter.wit"),
    )
...
fn reborn_runtime_http_egress_has_single_network_boundary() {
    ...
    let runtime_src_roots = [
        "crates/runtime/ironclaw_wasm/src",
        "crates/runtime/ironclaw_scripts/src",
        "crates/runtime/ironclaw_mcp/src",
        "crates/runtime/ironclaw_host_runtime/src",
    ];
...
fn workspace_root() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .and_then(|path| path.parent())
        .expect("architecture crate must live under crates/meta/ironclaw_architecture")
        .to_path_buf()
}
