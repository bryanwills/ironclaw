...
use ironclaw_memory_native::{
    ...
    MemoryDocumentScope, MemorySearchRequest, MemorySearchResult, MemoryServiceErrorKind,
    MemoryWriteOutcome,
};
use ironclaw_memory_native::{
...
#[tokio::test]
async fn native_search_drops_out_of_scope_and_protected_system_documents() {
    // The request scope is (tenant-native-memory, user-native-memory, no agent,
    // no project) from `invocation()`. The mock backend's `search` returns a
    // mix of: one exactly-in-scope normal document, four documents that each
    // differ on a single scope axis (tenant/user/agent/project), and one
    // in-scope but protected system document (HEARTBEAT.md). The defense-in-depth
    // `.filter(...)` in `NativeMemoryService::search` (scope equality +
    // `!is_protected_memory_path`) is solely responsible for dropping the bad
    // results — the backend deliberately hands all of them back. If that filter
    // were removed, all six would survive and the `len() == 1` assertion below
    // would fail.
    let service = NativeMemoryService::new(Arc::new(MockSearchBackend {
        results: vec![
            ...
            search_result_with_agent(
                ...
                None,
                Some("project-other"),
                "notes/wrong-project.md",
                0.8,
                "project leak",
            ),
            // In-scope but a protected system document — must be dropped.
            search_result(
                "tenant-native-memory",
                "user-native-memory",
                "HEARTBEAT.md",
                0.99,
                "protected heartbeat note",
            ),
        ],
        fail: false,
    }));
    ...
    // Only the exactly-in-scope, non-protected document survives the filter.
    assert_eq!(response.results.len(), 1);
    assert_eq!(response.results[0].path, "notes/allowed.md");
    assert_eq!(response.results[0].content, "in scope planning note");
}

#[tokio::test]
async fn native_tree_drops_out_of_scope_and_protected_system_documents() {
    // `tree` lists via the backend's `list_documents`. The request scope is
    // (tenant-native-memory, user-native-memory, no agent, no project). The mock
    // backend returns two in-scope normal paths, one out-of-scope path (different
    // tenant), and one in-scope protected system path (BOOTSTRAP.md). The
    // defense-in-depth `.filter(...)` in `NativeMemoryService::tree` (scope
    // equality + `!is_protected_memory_path`) is solely responsible for dropping
    // the out-of-scope and protected paths — the backend hands all of them back.
    // If that filter were removed, the out-of-scope and protected entries would
    // surface in the tree and the assertions below would fail.
    let service = NativeMemoryService::new(Arc::new(MockListBackend {
        paths: vec![
            document_path(
                "tenant-native-memory",
                "user-native-memory",
    ...
    // In-scope, non-protected paths are present.
    assert!(
        serialized.contains("alpha.md"),
        "in-scope path must appear in tree, got {serialized}"
    );
    assert!(
        serialized.contains("beta.md"),
        "in-scope path must appear in tree, got {serialized}"
    );
    // Out-of-scope and protected paths are dropped.
    assert!(
        !serialized.contains("leak.md"),
        "out-of-scope path must not appear in tree, got {serialized}"
    );
    assert!(
        !serialized.contains("BOOTSTRAP"),
        "protected system document must not appear in tree, got {serialized}"
    );
}
...
async fn native_context_retrieve_filters_cross_scope_results_and_hashes_snippet_refs() {
    ...
    assert_eq!(snippets[0].snippet_ref, "memory-snippet:e26104952dc2fb55");
}
...
/// `NativeMemoryService::tree` — not the backend — applies the scope +
/// protected-path filter. A dedicated mock is needed because `MockSearchBackend`
/// only implements `search`; `tree` lists through `list_documents`, which has a
/// distinct trait method and capability axis (`file_documents`).
struct MockListBackend {
    paths: Vec<MemoryDocumentPath>,
}
...
#[async_trait]
impl MemoryBackend for MockListBackend {
    fn capabilities(&self) -> MemoryBackendCapabilities {
        MemoryBackendCapabilities {
            file_documents: true,
            ..MemoryBackendCapabilities::default()
...
fn document_path(
    ...
    MemoryDocumentPath::new_with_agent(tenant, user, agent, project, path).unwrap()
}
