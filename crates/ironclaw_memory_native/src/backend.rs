...
// NOTE: the "already enforced" signal is no longer carried on the agnostic
// `MemoryContext`. It lives on the native-owned `MemoryBackendWriteOptions`
// (`prompt_safety_already_enforced`), which only the filesystem adapter sets
// before deferring backend re-enforcement. This keeps the bypass off the
// public agnostic contract so no foreign crate can forge it.
