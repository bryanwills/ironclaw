...
impl MemoryService for NativeMemoryService {
    async fn search(
        ...
        let results = self
            ...
            .await
            .map_err(|_| MemoryServiceError::operation())?
            .into_iter()
    ...
    async fn read(
        ...
        let content = String::from_utf8(bytes).map_err(|_| MemoryServiceError::operation())?;
    ...
    async fn profile_set(
        ...
        for _ in 0..MAX_MEMORY_PATCH_RETRIES {
            ...
            let mut doc: Map<String, Value> = match &current {
                Some(bytes) => {
                    serde_json::from_slice(bytes).map_err(|_| MemoryServiceError::operation())?
                }
                None => Map::new(),
            };
    ...
    async fn retrieve_context(
        ...
        if request.max_snippets == 0 || memory_context_disabled(&request.context_profile_id) {
            return Ok(Vec::new());
        }
        let (_, context) = self.scoped_context(&invocation)?;
        let search_request = MemorySearchRequest::new(&request.query)
...
impl NativeMemoryService {
    async fn patch_document(
        ...
        for _ in 0..MAX_MEMORY_PATCH_RETRIES {
            ...
            let existing = String::from_utf8(bytes).map_err(|_| MemoryServiceError::operation())?;
...
fn memory_context_disabled(context_profile_id: &str) -> bool {
    ...
    MEMORY_DISABLED_ALIASES.contains(&context_profile_id)
}
