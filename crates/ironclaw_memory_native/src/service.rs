...
/// System-managed memory documents that must never be surfaced to the model via
/// search/tree discovery; they have dedicated, protected write paths.
fn is_protected_memory_path(relative_path: &str) -> bool {
    relative_path == HEARTBEAT_PATH || relative_path == BOOTSTRAP_PATH
}
...
impl MemoryService for NativeMemoryService {
    async fn search(
        ...
        let results = self
            ...
            .into_iter()
            .filter(|result| {
                // Defense-in-depth: never surface a backend result outside the
                // request scope, with a non-finite score (would break JSON/tool
                // serialization), or a protected system document — even if a buggy
                // backend/index returns one.
                result.path.scope() == context.scope()
                    && result.score.is_finite()
                    && !is_protected_memory_path(result.path.relative_path())
            })
    ...
    async fn write(
        ...
        if request.target == "bootstrap" {
            ...
            return Ok(MemoryServiceWriteResponse {
                ...
                // Echo the user-facing alias, not the internal filename: BOOTSTRAP.md
                // is a protected system document that must not surface to the model.
                path: "bootstrap".to_string(),
                append: false,
                content_length: 0,
                replacements: None,
                message: Some("Bootstrap cleared.".to_string()),
            });
        }
    ...
    async fn tree(
        ...
        let mut paths = self
            ...
            .into_iter()
            .filter(|path| {
                path.scope() == &scope && !is_protected_memory_path(path.relative_path())
            })
    ...
    async fn retrieve_context(
        ...
        // Defense-in-depth: drop out-of-scope, non-finite, or protected-system
        // documents before they reach the prompt (matches search/tree).
...
fn memory_snippet_display_ref<'a>(parts: impl IntoIterator<Item = &'a str>) -> String {
    // Length-prefix each field so the pre-image is unambiguous — a structured
    // key rather than separator-joined concatenation, so no choice of field
    // values can collide at the encoding layer regardless of their contents.
    // The 64-bit FNV-1a digest keeps the ref compact and opaque (it must not
    // leak the document scope/path back to the model).
    let mut hash = FNV_OFFSET;
    for field in parts {
        let bytes = field.as_bytes();
        feed_hash(&mut hash, &(bytes.len() as u64).to_le_bytes());
...
fn memory_context_disabled(context_profile_id: &str) -> bool {
    ...
    // Fail closed: a disabled marker in any casing (e.g. "Memory-Disabled")
    // must disable retrieval rather than fall through and leak memory snippets.
    let normalized = context_profile_id.trim();
