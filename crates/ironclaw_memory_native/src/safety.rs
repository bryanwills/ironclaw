...
async fn emit_prompt_write_safety_event(
    ...
    if let Err(_error) = event_sink.record_prompt_write_safety_event(event).await {
        // The sink error's reason is free-form and may carry backend
        // paths/details; log only the sanitized operation/source context.
        tracing::debug!(
            target: "ironclaw::memory::prompt_write_safety",
            operation = %check.operation,
            source = %check.source,
