...
//! in `crates/state/ironclaw_memory/src/` as of 2026-05-12). The followup PR
...
mod libsql_e2e {
    ...
    #[cfg_attr(
        ...
        ignore = "requires `.system/engine/orchestrator/*` registration in the protected-path registry. Empirically (2026-05-12) no such registration exists in #3180 as merged (no `orchestrator` symbol in `crates/state/ironclaw_memory/src/`). Stays gated for the followup PR that adds the registration."
    )]
    ...
    #[cfg_attr(
        not(feature = "pr3180-ready"),
        ignore = "requires `ensure_path_matches_context`/`ensure_scope_matches_context` fail-closed guards. Empirically (2026-05-12) no such function exists in `crates/state/ironclaw_memory/src/` after #3180 — the substrate does not yet enforce that path scope matches context scope. Stays gated for the followup PR that adds the guard."
    )]
