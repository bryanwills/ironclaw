...
#[tokio::test]
async fn filesystem_list_deferred_busy_messages_limit_caps_results() {
    let backend = Arc::new(InMemoryBackend::new());
    let scoped = scoped_threads_fs_at(backend, "tenant-ldb-limit", "alice");
    let service = FilesystemSessionThreadService::new(scoped);
    let thread = service
        .ensure_thread(EnsureThreadRequest {
    ...
    // limit=2 — oldest two returned, sequence-ascending.
    let result = service
        .list_deferred_busy_messages(ListDeferredBusyMessagesRequest {
            scope: scope("ldb-limit"),
            thread_id: thread.thread_id.clone(),
...
#[tokio::test]
async fn filesystem_list_deferred_busy_messages_after_sequence_filters_earlier_records() {
    let backend = Arc::new(InMemoryBackend::new());
    let scoped = scoped_threads_fs_at(backend, "tenant-ldb-aseq", "alice");
    let service = FilesystemSessionThreadService::new(scoped);
    let thread = service
        .ensure_thread(EnsureThreadRequest {
            scope: scope("ldb-aseq"),
            thread_id: None,
            created_by_actor_id: "actor-a".into(),
            title: None,
            metadata_json: None,
    ...
    let _ = rec_a; // rec_a captured but not used in filesystem variant (no combined cursor+limit case)
    let empty = service
        .list_deferred_busy_messages(ListDeferredBusyMessagesRequest {
            scope: scope("ldb-aseq"),
            thread_id: thread.thread_id,
