...
#[derive(Debug, Clone)]
pub struct HostOAuthProviderSpec {
...
impl HostOAuthProviderSpec {
    /// Stable provider identifier. Exposed so composition-side build-input
    /// builders can de-duplicate provider backend configs by id without
    /// reaching into the provider's HTTP-internal fields.
...
#[derive(Clone)]
pub struct HostOAuthProviderClient {
