...
pub(super) async fn dispatch(
    ...
    // Authorize the `/memory` write FIRST so an unauthorized caller fails
    // closed with the filesystem-denied error instead of leaking field-level
    // validation feedback about untrusted profile input.
...
mod tests {
    ...
    #[tokio::test]
    async fn profile_set_authorizes_memory_mount_before_parsing_untrusted_input() {
        // An unauthorized caller (no `/memory` mount) must fail closed with the
        // filesystem-denied error BEFORE the untrusted profile JSON is parsed —
        // even when that JSON is invalid. Otherwise the caller learns about
        // field-level validation feedback it is not authorized to receive.
        let memory_service = Arc::new(RecordingProfileMemoryService::default());
        let state = MemoryCapabilityState::with_memory_service_for_test(memory_service.clone());
        // Invalid input AND no mount: authorization must win.
        let request = profile_set_request_with_mounts(json!({"always_approve": true}), None);

        let err = dispatch(&state, &request)
            .await
            .expect_err("unauthorized profile_set must be rejected");

        assert!(
            matches!(
                err.kind(),
                Some(ironclaw_host_api::RuntimeDispatchErrorKind::FilesystemDenied)
            ),
            "authorization (FilesystemDenied) must precede input validation (InputEncode)"
        );
        let seen = memory_service
            .seen
            .lock()
        ...
        assert!(
            seen.is_empty(),
            "unauthorized profile_set must not call facade"
        );
    }
}
