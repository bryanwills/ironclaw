...
use ironclaw_memory::{
    MemoryInvocation, MemoryService, MemoryServiceContextRequest, MemoryServiceError,
    MemoryServiceErrorKind,
};
...
const MAX_MEMORY_CONTEXT_TOTAL_BYTES: usize = 8 * 1024;
        ...
        let snippets = self
            ...
            .retrieve_context(
                ...
                MemoryServiceContextRequest {
                    ...
                    context_profile_id: request.context_profile_id.as_str().to_string(),
                },
            )
            .await
...
fn admit_memory_context_snippet(
    ...
    {
        tracing::warn!("dropping memory context snippet with invalid ref");
        return None;
    }
    ...
    {
        tracing::warn!("dropping memory context snippet without host-accepted wrapper");
        return None;
    }
    if snippet.model_content.len() > MAX_MEMORY_CONTEXT_SNIPPET_BYTES {
        tracing::warn!("dropping oversized memory context snippet");
        return None;
    }
    let safe_summary = LoopSafeSummary::new(snippet.safe_summary.clone()).ok()?;
    let model_content = LoopSafeSummary::new(snippet.model_content).ok()?;
    let next_total = total_bytes.saturating_add(model_content.as_str().len());
    if next_total > MAX_MEMORY_CONTEXT_TOTAL_BYTES {
        tracing::warn!("dropping memory context snippet over aggregate budget");
        return None;
    }
