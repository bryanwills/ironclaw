...
#[tokio::test]
async fn host_http_egress_helper_reuses_staged_credentials_until_capability_cleanup() {
    let scope = sample_scope();
    let capability_id = sample_capability_id();
    let handle = SecretHandle::new("api-token").unwrap();
    ...
    egress
        .execute(request_with_staged_credential(scope, capability_id, handle))
        .await
        .expect("second request in same invocation should reuse staged credential");
