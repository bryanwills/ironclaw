...
| WebUI extension setup | `/api/webchat/v2/extensions/{extension_name}/setup` validates typed `ExtensionName` and returns lifecycle projection. | `LifecycleProductFacade::execute` with `ExtensionConfigure` action. | Projection wired; no setup side effects yet. |
| Skill install/remove/list | Existing first-party `builtin.skill_*` capability uses Reborn skill management filesystem logic; local-dev composition now wraps the same skill management implementation behind `LifecycleProductFacade`. | `LifecycleProductFacade` over skill management. | Local-dev wired; production lifecycle store/composition still follow-up. |
| Extension install/activate/remove | Lower-level extension manifest, installation, and lifecycle services exist. | Extension lifecycle/config/package services behind facade. | Not production-wired through the facade yet. |
